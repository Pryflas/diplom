...
def build_model(hp):
    ...
    # Сеть считает все метрики, но оптимизироваться будет по AUC
    model.compile(
...
# ТЮНЕР НАСТРОЕН НА ROC-AUC (val_AUC)
tuner = kt.Hyperband(
    build_model,
    objective=kt.Objective("val_AUC", direction="max"), 
    max_epochs=50,
    factor=3,
    directory='tuner_logs_auc_roc',
    project_name='diabetes_tuning_auc_roc', 
    overwrite=True
)

# КОЛЛБЭКИ НАСТРОЕНЫ НА ROC-AUC (val_AUC)
...
# КОЛЛБЭК НАСТРОЕН НА ROC-AUC (val_AUC)
...
best_model.save('best_disease_risk_model_auc_roc.keras')
...
plt.tight_layout()
plt.savefig('shap_summary_auc_roc.png')
print("График 'shap_summary_auc_roc.png' успешно сохранен в папке проекта!")