...
# === ДОПОЛНИТЕЛЬНЫЕ МЕТРИКИ (ДЛЯ НАБЛЮДЕНИЯ) ===
...
class CustomScoreCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if logs is not None:
            ...
            # print(f" — val_custom_score: {custom_score:.4f}") # Раскомментируй, если хочешь видеть в логах каждую эпоху
...
def build_model(hp):
    ...
    model.compile(
        ...
        metrics=[
            ...
            tf.keras.metrics.AUC(curve='PR', name='pr_auc'), # Именно эта метрика будет оптимизироваться
...
# ИЗМЕНЕНИЕ: ТЮНЕР НАСТРОЕН НА PR-AUC (val_pr_auc)
tuner = kt.Hyperband(
    build_model,
    objective=kt.Objective("val_pr_auc", direction="max"), 
    max_epochs=50,
    factor=3,
    directory='tuner_logs_pr_auc',
    project_name='diabetes_tuning_pr_auc', 
    overwrite=True
)

# ИЗМЕНЕНИЕ: КОЛЛБЭКИ НАСТРОЕНЫ НА PR-AUC (val_pr_auc)
...
# ИЗМЕНЕНИЕ: КОЛЛБЭК НАСТРОЕН НА PR-AUC (val_pr_auc)
reduce_lr = ReduceLROnPlateau(monitor='val_pr_auc', mode='max', factor=0.2, patience=5, min_lr=1e-6, verbose=1)
...
best_model.save('best_disease_risk_model_pr_auc.keras')
...
plt.tight_layout()
plt.savefig('shap_summary_pr_auc.png')
print("График 'shap_summary_pr_auc.png' успешно сохранен в папке проекта!")